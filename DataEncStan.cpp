...
class Encryptof:public Des
{
  public:
 

      char * st;
      friend istream & operator>>(istream&,const Encryptof&);
         
       char * Encrypt(char *);  

};
istream & operator>>(istream& din,const Encryptof& d1)
{
   din>>d1.st;
   return din;
}

class Decryptof:public Des
{
  public:
         char * Decrypt(char *);
...
char * Encryptof::Encrypt(char * text)
{
...
char * Decryptof::Decrypt(char * TEXT1)
{
    int i,a1,j,nB,m,iB,k,K,B[8],n,t,d,round;
    char *Text=new char[1000];
    unsigned char ch;
...
int main()
{
    ...
    cout<<"--------------------------------------------------DATA ENCRYPTION STANDARD---------------------------------------------------------"<<endl;
     cout<<"Enter a string : ";
    cin>>d1;             
    str1=d1.Encrypt(d1.st);
    cout<<"\ni/p Text: "<<d1.st<<endl;
    cout<<"\nCypher  : "<<str1<<endl;
    ofstream fout("out2_fil.txt"); 
    fout<<str1; 
    fout.close();
